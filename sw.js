/* She service worker v7 — fresh HTML, offline fallback for app assets */
const CACHE='she-shell-v7';
...
async function cacheShell(){const c=await caches.open(CACHE);for(const url of SHELL){try{const r=await fetch(url,{cache:'no-store'});if(r.ok)await c.put(url,r)}catch(_){}}}
...
self.addEventListener('fetch',e=>{const req=e.request;if(req.method!=='GET')return;const u=new URL(req.url);if(!own(u))return;
 if(req.mode==='navigate'){
  e.respondWith((async()=>{try{const r=await fetch(new Request(req,{cache:'no-store'}));if(r.ok){const c=await caches.open(CACHE);c.put(u.pathname,r.clone()).catch(()=>{});}return r}catch(_){const c=await caches.match(u.pathname);return c||await caches.match('/chats.html')||new Response('Offline',{status:503})}})());return;
 }
